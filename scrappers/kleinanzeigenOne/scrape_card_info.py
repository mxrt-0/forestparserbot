...
from scrappers.kleinanzeigenOne.utils import fetch_data, urls_generator
from scrappers.utils import proxy_list
from bot.database.database import execute_query, fetch_data
...
async def scrape_urls(urls, limiter, session, proxies):
    ...
    for html in htmls:
        ...
        try:
            ...
            """
            await execute_query(query, values=cards_info, target="many")
        except Exception as e:
...
async def fetch_views(limiter, session, proxies):
    ...
    try:
        query = """
            ...
            WHERE views IS NULL;
        """
        cards_info = await fetch_data(query, target="all")
    except Exception as e:
        print(
...
async def scrape_views(proxies, limiter, session):
        ...
        try:
            query = """
                ...
                WHERE id = $1;
            """
            await execute_query(query, values=cards_w_views, target="many")
        except Exception as e:
